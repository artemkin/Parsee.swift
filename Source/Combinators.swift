...
func >>=<A, B>(a: Parser<A>, f: A -> Parser<B>) -> Parser<B> {
...
// map
infix operator >>| { associativity left }
func >>|<A, B>(a: Parser<A>, f: A -> B) -> Parser<B> {
    return a >>= { a in parseOK(f(a)) }
}
...
infix operator <|> { associativity left }
func <|><T>(a: Parser<T>, b: Parser<T>) -> Parser<T> {
...
// fmap or <$>
infix operator <^> { associativity left }
func <^><A, B>(f: A -> B, a: Parser<A>) -> Parser<B> {
    return Parser<B> { i in
        switch a.run(i) {
        case let (.OK(data), input):
